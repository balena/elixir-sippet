defmodule Sippet.Message do
  ...
  @type uri_params :: {display_name :: binary, uri :: URI.t(), params}

  @type name_uri_params :: {display_name :: binary, uri :: URI.t(), params}
  ...
  def to_response(
    ...
    response =
      if status_line.status_code > 100 and not Map.has_key?(elem(response.headers.to, 2), "tag") do
  ...
  values under `header`.
  """
  @spec get_and_update_header(t, header, (value -> {get, value} | :pop)) :: {get, t}
        when get: value
  ...
  def parse!(data) do
    case parse(data) do
      ...
      {:error, reason} ->
        raise ArgumentError,
              "cannot convert #{inspect(data)} to SIP " <> "message, reason: #{inspect(reason)}"
    end
  end
  ...
  defp do_header_value({{major, minor}, protocol, {host, port}, %{} = parameters})
       when is_integer(major) and is_integer(minor) and is_binary(host) and is_integer(port) do
    [
      "SIP/",
  ...
  defp has_valid_body(message) do
    case message.headers do
      %{content_length: content_length} ->
        if message.body != nil and byte_size(message.body) == content_length do
          :ok
        else
