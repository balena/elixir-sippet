defmodule Sippet.Transports.TCP.Handler do
  ...
  def connection_process(parent, ref, transport, opts) do
    proxy_info =
      case opts |> Keyword.get(:proxy_header, opts, false) do
        true ->
    ...
    init(parent, ref, socket, transport, proxy_info, opts)
  end
  ...
  defp parse_body(
         buffer,
         %{clen: clen, header: header, peer: {from_ip, from_port}, sippet: sippet} = state
       ) do
    <<body::binary-size(clen), rest::binary>> = buffer
    message = header <> body

    Sippet.Router.handle_transport_message(sippet, message, {:tcp, from_ip, from_port})
  ...
  defp handle_call(
         ...
         {from, tag},
         %{socket: socket, transport: transport, peer: {peer_ip, peer_port}, sippet: sippet} =
           state
       ) do
    Logger.debug([
      "sending message to #{stringify_ipport(peer_ip, peer_port)}/tcp",
      ", #{inspect(key)}"
    ])
    ...
    case transport.send(socket, Message.to_iodata(message)) do
      ...
      {:error, reason} ->
        Logger.warn(
          "tcp transport error for #{stringify_ipport(peer_ip, peer_port)}: #{inspect(reason)}"
        )
